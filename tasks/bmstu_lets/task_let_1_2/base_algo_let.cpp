...
std::vector<int> positive_numbers(const std::vector<int>& nums) {
  ...
  std::copy_if(nums.begin(), nums.end(), std::back_inserter(result), [](int value) { return value > 0; });
  return result;
}

void sort_positive_numbers(std::vector<int>& nums) {

  auto partition_point = std::partition(nums.begin(), nums.end(), [](int num) {
      return num > 0;
  });
...
void replace_negative_numbers(std::vector<int>& nums) {
  std::replace_if(nums.begin(), nums.end(), [](int value) { return value < 0; }, 0);
}
...
void sort_students_by_age(std::vector<Student>& data) {
  std::sort(data.begin(), data.end(), [](const Student& a, const Student& b) { return a.age < b.age; });
}
